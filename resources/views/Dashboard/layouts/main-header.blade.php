        ...
        <div class="main-header-right">
                           ...
                           aria-expanded="false">
                            @if (App::getLocale() == 'ar')
                                <span class="avatar country-Flag mr-0 align-self-center bg-transparent"><img
                                        src="{{URL::asset('Dashboard/img/flags/sa_flag.jpg')}}" alt="img"></span>
                                <strong
            ...
            <div class="nav nav-item  navbar-nav-right ml-auto">
                ...
                <div class="dropdown nav-item main-header-notification">
                    ...
                    {{--  <p data-count="0" class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 notif-count"> {{App\Models\Notification::where ('user_id', auth()->user()->name)->where ('reader_status',0)->count()}}</p>--}}
                    {{--  يوجد طريقة اخرى باستخدام scope نكتبها في ال models--}}
                        ...
                        {{--
                         data-count="{{App\Models\Notification::CountNotification(auth()->user()->name)->count()}} بجيب العدد الموجود في قاعدة البيانات ليتم الزيادة 1 على العدد الموجود
                        CountNotification from scopeCountNotification in model and remove scope--}}
                        </div>
...
{{--                                    <a href=( wire:click="print({{ $single_invoice->id }})" ) >{{ $single_invoice->Patient->name }}</a>--}}
...
<script>
    ...
    var notificationsWrapper   = $('.dropdown-notifications'); // we add dropdown-notifications in   <div class="dropdown-menu
    var notificationsCountElem = notificationsWrapper.find('p[data-count]'); // تم تعريفها في العداد
    var notificationsCount     = parseInt(notificationsCountElem.data('count'));

    var notifications          = notificationsWrapper.find('h4.notification-label'); // notification-label the name of class in h4 ..   <h4 class="notification-label
    ...
    // var pusher = new Pusher('31d13d24dfcabb567e85', {
    //     cluster: 'mt1'
    // });  // for public



    // var channel = pusher.subscribe('create-invoice');   // for public channel
    ...
    // channel.bind('App\\Events\\CreateInvoice', function(data) {  // for public

        // alert(JSON.stringify(data.patient_id));

    Echo.private('create-invoice.{{ auth()->user()->id }}').listen('.create-invoice', (data) => { // for private


        var newNotificationHtml =`
            <h4 class="notification-label mb-1">`+data.message+data.patient+`</h4>
            <div class="notification-subtext">`+data.created_at+`</div>`;
        new_message.show(); //  <div class = "new_message">
        notifications.html(newNotificationHtml);
        notificationsCount += 1;
        notificationsCountElem.attr('data-count', notificationsCount);
        notificationsWrapper.find('.notif-count').text(notificationsCount); // copy notif-count to <p data-count="0"...
