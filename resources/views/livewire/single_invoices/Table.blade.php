...
<div class="table-responsive">
    <table class="table text-md-nowrap" id="example1" data-page-length="50"style="text-align: center">
        ...
        <tbody>
        @foreach ($single_invoices as $single_invoice)   {{--تم تعريفها في render--}}
            <tr>
                <td>{{ $loop->iteration}}</td>
                <td>{{ $single_invoice->Service->name }}</td>
{{--                <td>{{ $single_invoice->Patient->name }}</td>--}}
                <td><a href=( wire:click="print({{ $single_invoice->id }})" ) >{{ $single_invoice->Patient->name }}</a></td>
                ...
                <td>
                    ...
                    <button wire:click="print({{ $single_invoice->id }})" class="btn btn-warning btn-sm" ><i class="fas fa-print"></i></button>
                </td>
            </tr>
