...
class CreateGroupServices extends Component
{
    ...
    public function saveGroup()
    {
        ...
        if($this->updateMode){
            ...
            $Groups = Group::find($this->group_id); // group_id اخدها من كود التعديل وتم تعريفها بداية الصفحة
            $total = 0;
            foreach ($this->GroupsItems as $groupItem) {
                if ($groupItem['is_saved'] && $groupItem['service_price'] && $groupItem['quantity']) {
                    // الاجمالي قبل الخصم
    ...
    public function edit($id) //  عرفناها فوق
    {
        $this->show_table = false;
        $this->updateMode = true; //  عرفناها فوق
        $group = Group::where('id',$id)->first();
        $this->group_id = $id;  //  عرفناها فوق

        $this->reset('GroupsItems', 'name_group', 'notes'); // clean
        ...
        foreach ($group->service_group as $serviceGroup)
        {
            $this->GroupsItems[] = [
                ...
                'quantity' => $serviceGroup->pivot->quantity, // pivot its go to withpivot in relation model
                'is_saved' => true,
