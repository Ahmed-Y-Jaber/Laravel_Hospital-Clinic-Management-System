...
class SingleInvoices extends Component
{
    ...
    //for pusher
    public $username,$message,$patient;
    ...
    public function render()
    {
        return view('livewire.single_invoices.single-invoices', [
            'single_invoices'=>Invoice::where('invoice_type',1)->get(), // 1  يعني فاتورة مفردة
    ...
    public function get_section()
    {
        ...
        // with('section') relation name from model doctor
    ...
    public function edit($id){
        ...
        $this->single_invoice_id = $single_invoice->id;  // single_invoice_id حطيناها في متغير فوق
        ...
        if($this->type == 1){
            ...
            try {
                ...
                if($this->updateMode){
                    ...
                    // روح ع جدول الترانسليشن النيم المعروض هات ال اي دي تبعه
                    // حتى يخزن id مش الترجمة
                    ...
                    $fund_accounts = FundAccount::where('invoice_id',$this->single_invoice_id)->first(); // حساب الصندوق
                    ...
                    $single_invoices->total_with_tax = $single_invoices->price -  $single_invoices->discount_value + $single_invoices->tax_value;
                    $single_invoices->type = $this->type;
                    $single_invoices->invoice_status = 1;  // يعني لسا ما راحت عل الدكتور
                    $single_invoices->save();

                    $fund_accounts = new FundAccount(); // حساب الصندوق
                    $fund_accounts->date = date('Y-m-d');
                    $fund_accounts->invoice_id = $single_invoices->id;
                    $fund_accounts->Debit = $single_invoices->total_with_tax;
                    $fund_accounts->credit = 0.00;
                    ...
                    $data=[
                        ...
                        'doctor_id'=>$this->doctor_id, //for private channel

                        // we determine doctor_id in CreateInvoice
                    ];


//                    event(new MyEvent('hello world','ahmed'));
