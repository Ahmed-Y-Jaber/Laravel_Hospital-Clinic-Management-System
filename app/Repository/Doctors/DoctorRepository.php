...
class DoctorRepository implements DoctorRepositoryInterface
{
    ...
    public function index()
    {
        ...
        // any relation (many to many) must return (with)
    ...
    public function create()
    {
        ...
        return view('Dashboard.Doctors.add',compact('sections','appoinments'));
    }
    ...
    public function store($request){
        ...
        try {
           ...
           // $doctors->appointments =implode(",",$request->appointments); // implode عشان تعمل ، في الاراي مثل السبت، الاحد،وهكذا
            $doctors->save();


            $doctors->doctorappointments()->attach($request->appoiment);
            $doctors->save();

            //Upload img          // عشان هاي عملنا ملف traits
    ...
    public function destroy($request)
    {
      if($request->page_id==1){
       ...
       if($request->filename){    //filename  == name="filename"  from delete.blade

         $this->Delete_attachment('upload_image','doctors/'.$request->filename,$request->id,$request->filename);
        // Delete_attachment($disk,$path,$id,$filename)
        //  $this->Delete_attachment  من trait
        // upload_image اسم الديسك
        // 'doctors/'.$request->filename المسار بالكامل مع اسم الصورة
        // $request->id الجاية من الفورم
       }
     ...
     $delete_select_id = explode(",", $request->delete_select_id); // explode  عشان يعملها مصفوفة ويحط ، بين كل اي دي
     //$delete_select_id  = name="delete_select_id" from delete_select.blade
